...
!function (window) {
    ...
    CitySelect.prototype.open = function () {
        ...
        if (defaultSet.provance) {
            _this.setNavTxt(0, defaultSet.provance,defaultSet.pid);
        } else {
        ...
        if (defaultSet.city) {
            _this.loadCity();
            _this.setNavTxt(1, defaultSet.city,defaultSet.cid);
        }

        if (defaultSet.area) {
            _this.loadArea();
            _this.ForwardView(false);
            _this.setNavTxt(2, defaultSet.area,defaultSet.aid);
        }
    ...
    CitySelect.prototype.setNavTxt = function (index, txt, id) {
        ...
        switch(index){
            case 0:
                $nav.eq(index).attr('pid',id);
                break;
            case 1:
                $nav.eq(index).attr('cid',id);
                break;
            case 2:
                $nav.eq(index).attr('aid',id);
                break;
        }
    ...
    CitySelect.prototype.bindItemEvent = function () {
        ...
        $cityElement.on('click.ydui.cityselect', '.cityselect-item-box a', function () {
            ...
            switch (tag){
                case 0:
                    _this.setNavTxt(tag, $this.text(),$this.attr("pid"));
                    break;
                case 1:
                    _this.setNavTxt(tag, $this.text(),$this.attr("cid"));
                    break;
                case 2:
                    _this.setNavTxt(tag, $this.text(),$this.attr("aid"));
                    break;
            }
            ...
            if (tag == 0) {
                ...
                defaultSet.pid = $nav.eq(0).attr('pid');
                defaultSet.city = $nav.eq(1).html();
                defaultSet.cid = $nav.eq(1).attr('cid');
                defaultSet.area = $nav.eq(2).html();
                defaultSet.aid = $nav.eq(2).attr('aid');
                _this.returnValue();
            }
        });
    };
    ...
    CitySelect.prototype.loadProvance = function () {
        ...
        $.each(_this.citys, function (k, v) {
            arr.push($('<a  pid="'+ v.id+'"class="' + (v.n == _this.defaultSet.provance ? 'crt' : '') + '" href="javascript:;"><span>' + v.n + '</span></a>').data({
                citys: v.c,
                tag: 0
            }));
        });
    ...
    CitySelect.prototype.loadCity = function () {
        ...
        $.each(cityData, function (k, v) {
            arr.push($('<a cid="'+ v.id+'"class="' + (v.n == _this.defaultSet.city ? 'crt' : '') + '" href="javascript:;"><span>' + v.n + '</span></a>').data({
                citys: v.a,
                tag: 1
            }));
        });
    ...
    CitySelect.prototype.loadArea = function () {
        ...
        $.each(areaData, function (k, v) {
            ...
            arr.push($('<a aid="" class="' + (v == _this.defaultSet.area ? 'crt' : '') + '" href="javascript:;"><span>' + v + '</span></a>').data({tag: 2}));
        });
